...
#             """
# #     return """"
# # NETWORK LATENCY FIX TASK

# # Use only real tool data. Do not assume. Analyze before acting.

# # STEPS:
# # 1. Call get_organization_uplinks_statuses → capture BEFORE WAN distribution.
# # 2. Call get_device_loss_and_latency_history → capture latency per device & trends.
...
# #     1. Combine all devices from all WANs into a single list.
# #     2. Shuffle or sort devices to evenly mix high- and low-latency devices.
# #     3. Redistribute devices across WANs:
# #         - Ensure each WAN has roughly equal number of devices.
# #         - Each WAN should have a mix of high- and low-latency devices.
# #     4. Move devices using update_uplink as needed.

# # - High-Latency Check After Redistribution:
# #     1. If device counts are balanced but latency >100ms persists:
# #         - Identify worst-performing devices.
# #         - Move subset to a new WAN (using update_appliance_settings or update_uplink).
# #     2. Priority Devices (VoIP/video):
# #         - Move them first to lowest-latency WAN.
# #     move its devices to healthy WANs.

# # - Trend-Based Rerouting:
# #     - If latency is rising rapidly (even below threshold) → reroute proactively.

# # FINAL STEPS:
# # 1. Call get_organization_uplinks_statuses again → capture AFTER WAN distribution.

# #     IMPORTANT:
# #     - Always mix high- and low-latency devices across WANs.
# #     - Use real tool data from get_organization_uplinks_statuses and get_device_loss_and_latency_history.
# #     - Generate pointwise, readable, actionable report.
# #     - Do not just say “execute”—assume you are the network orchestrator and take the actions automatically.
# #     """
...
#     use_gemini = False 
#     use_glm =True # Set to True for Gemini, False for DeepSeek
...
#             # GLM-4.5 via z.ai using Anthropic-compatible API
#         elif use_glm:
#             try:
...
#             handle_parsing_errors=True,  # Better error handling than MCPAgent
#             return_intermediate_steps=True  # Clean output for latency monitoring
#         )
...
#original client with the  langchain 
...
2. High-latency devices are rerouted first to the best available WAN.  
3. Low-latency devices are moved next if needed.  

# TOOLS YOU MUST USE (MANDATORY):
- get_organization_uplinks_statuses → Get current WAN distribution data.  
- get_device_loss_and_latency_history → Get latency data for devices.  
- update_uplink → Move devices to the best WAN.  

# STRICT STEPS:
1. Call get_organization_uplinks_statuses → Capture BEFORE distribution.  
2. Call get_device_loss_and_latency_history → Identify latency per device.  
3. DECIDE and MOVE devices using update_uplink.  
    IMPORTANT: Do not wait for permission. You must reroute devices based on latency and WAN balance.  
   - First move high-latency devices to better WANs.  
   - Then move low-latency devices until WANs are balanced.  
4. Call get_organization_uplinks_statuses again → Capture AFTER distribution.  

# FINAL OUTPUT FORMAT (MANDATORY):
** Analysis **  
(Summarize the insights from both tools with actual data)  

** Decision **  
(Clearly explain which devices were selected and why)  

** Modified **  
(List the changes you applied with update_uplink)  

** Impact **  
(Show before vs after WAN distribution counts and improvements in latency)  

# REMINDER:
- You are fully authorized to ACT.  
- Do not only report.  
- Must perform rerouting decisions automatically.  
"""
